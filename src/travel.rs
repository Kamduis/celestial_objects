...
impl SpaceTravel {
	...
	/// * `acceleration` The acceleration of the spacecraft.
	pub fn from_dist_accel( distance: Length, acceleration: Acceleration ) -> SpaceTravel {
		Self {
			distance,
			acceleration,
			speed_limit: None,
	...
	pub fn duration( &self ) -> TimeDelta {
		let accel_si = self.acceleration.as_f64();
...
mod tests {
	...
	fn test_duration_spaceflight() {
		let flight_0 = SpaceTravel::from_dist_accel( Length::from_meter( 100.0 ), 10.0.into() );
		assert_eq!( flight_0.duration(), TimeDelta::new( 6, 324555320 ).unwrap() );
		assert_eq!( flight_0.speed_max(), 31.622776601683796 );

		let flight_1 = SpaceTravel::from_dist_accel( Length::from_meter( 100.0 ), 10.0.into() )
		...
		let flight_2 = SpaceTravel::from_dist_accel( Length::from_meter( 200.0 ), 5.0.into() );
		assert_eq!( flight_2.duration(), TimeDelta::new( 12, 649110640 ).unwrap() );
		assert_eq!( flight_2.speed_max(), 31.622776601683796 );

		let flight_3 = SpaceTravel::from_dist_accel( Length::from_meter( 200.0 ), 5.0.into() )
			.with_maneuver( SpaceTravelManeuver::Intercept );
		assert_eq!( flight_3.duration(), TimeDelta::new( 8, 944271909 ).unwrap() );
		assert_eq!( flight_3.speed_max(), 44.721359549995796 );

		let flight_4 = SpaceTravel::from_dist_accel( Length::from_meter( 100.0 ), 10.0.into() )
			.with_speed_limit( 1.0 );
		assert_eq!( flight_4.duration(), TimeDelta::new( 100, 100000000 ).unwrap() );
		assert_eq!( flight_4.speed_max(), 1.0 );

		let flight_5 = SpaceTravel::from_dist_accel( Length::from_meter( 100.0 ), Acceleration::from_g( 1.0 ) );
		assert_eq!( flight_5.duration(), TimeDelta::new( 6, 386599135 ).unwrap() );
		assert_eq!( flight_5.speed_max(), 31.31557120666969 );
	}
}
